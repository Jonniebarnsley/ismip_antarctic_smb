...
the data onto the ISMIP 8km 761x761 South polar stereographic grid. Then computes SMB using
the formula:
...
# local imports
from regrid_CMIP_to_ISMIP import Regridder
...
def regrid_and_concat(data_dir: Path) -> Dataset:
    ...
    with Regridder(method="bilinear", annual_mean=True) as rg:
        for file in sorted(data_dir.glob("*.nc")):
            print("   ", file.stem)
            regridded = rg.regrid_CMIP_to_ISMIP(file)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Preprocess CMIP SMB data by calculating annual means, regridding to "
                    "ISMIP 8km grid, and computing SMB."
    )
